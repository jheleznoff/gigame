      ...
      nodes: [
        ...
        },
        {
          ...
          data: {
            ...
            knowledge_base_id: '',
            prompt:
              'Ты получил параметры новой закупки (результат предыдущего шага) и релевантные фрагменты из базы знаний — исторические данные по аналогичным прошлым сделкам.\n\n' +
              'Проведи сравнительный анализ:\n\n' +
              '## 1. Найденные аналоги\n' +
              'Перечисли найденные в базе похожие сделки (предмет, объём, поставщик, цена, срок, дата). Оцени степень похожести: высокая / средняя / низкая.\n\n' +
              '## 2. Цена\n' +
              '- Средняя цена по аналогам (за единицу и общая)\n' +
              '- Минимум / максимум / медиана\n' +
              '- Текущая цена: в норме / выше рынка на X% / ниже рынка на X%\n' +
              '- Вердикт: 🟢 типично / 🟡 требует уточнения / 🔴 аномально\n\n' +
              '## 3. Сроки поставки\n' +
              '- Типичный срок по аналогам\n' +
              '- Текущий срок: норма / короче / длиннее\n' +
              '- Вердикт: 🟢 / 🟡 / 🔴\n\n' +
              '## 4. Поставщик\n' +
              '- Работали ли с этим поставщиком раньше?\n' +
              '- Если да — были ли проблемы, какие цены и сроки давал\n' +
              '- Если нет — есть ли в аналогах предупреждающие сигналы по новым поставщикам\n\n' +
              '## 5. Особые условия\n' +
              'Нетипичные условия оплаты, гарантии, исключения — сравни с историей.\n\n' +
              '## 6. Выводы и рекомендации\n' +
              '- Что в текущей сделке в пределах нормы\n' +
              '- Где ожидаются отклонения и какие\n' +
              '- Что стоит дополнительно уточнить у поставщика\n' +
              '- Общий вердикт: 🟢 можно согласовать / 🟡 требуется обсуждение / 🔴 нужны дополнительные обоснования\n\n' +
              'Если в базе знаний не нашлось релевантных аналогов — честно напиши об этом и не выдумывай средние значения.',
          },
        },
        {
          id: 'node_3',
      ...
      nodes: [
        ...
        },
        {
          ...
          data: {
            ...
            knowledge_base_id: '',
            prompt:
              'Ты получил список извлечённых позиций (результат предыдущего шага) и релевантные фрагменты из корпоративного справочника номенклатуры (база знаний).\n\n' +
              'Для КАЖДОЙ извлечённой позиции выполни:\n\n' +
              '## 1. Сопоставление с каталогом\n' +
              'Найди в релевантных фрагментах КБ наиболее подходящую каноническую позицию. Укажи:\n' +
              '- Исходное наименование (как было)\n' +
              '- Каноническое наименование из справочника (если нашлось)\n' +
              '- Код / артикул из справочника\n' +
              '- Уверенность: высокая (точное совпадение) / средняя (похожее) / низкая (только категория) / нет в справочнике\n\n' +
              ...
              'Отдельный список позиций, которых НЕТ в справочнике. Для каждой предложи:\n' +
              '- Какую категорию каталога она напоминает\n' +
              '- Нужно ли её добавить в справочник или это одноразовая закупка\n\n' +
              '## 4. Сводная таблица\n' +
              '| № | Каноническое наименование | Код | Кол-во | Цена ед. | Уверенность | Источники (исходные названия) |\n' +
              '|---|---|---|---|---|---|---|\n\n' +
              '## 5. Итоги\n' +
              '- Всего исходных позиций: N\n' +
              '- Объединено дублей: N\n' +
              '- Сопоставлено с каталогом: N (N%)\n' +
              '- Не опознано: N\n\n' +
              'Если база знаний не нашла релевантных фрагментов — честно напиши, что справочник не дал совпадений, и верни только исходные позиции в виде таблицы.',
          },
        },
        {
          id: 'node_3',
